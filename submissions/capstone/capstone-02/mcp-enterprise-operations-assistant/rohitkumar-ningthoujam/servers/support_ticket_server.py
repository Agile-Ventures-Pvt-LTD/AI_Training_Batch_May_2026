...
def search_tickets(service_name: str = None, priority: str = None, status: str = None, limit: int = 50) -> dict:
    """Search support tickets using predefined filters for service, priority, status, and limit."""
...
def get_ticket_details(ticket_id: str) -> dict:
    """Get full details of one support ticket by ticket ID."""
    conn = _get_connection()
    cursor = conn.cursor()
    cursor.execute(
        "SELECT ticket_id, service_name, priority, status, subject, description, created_at, customer_impact, assigned_group FROM tickets WHERE ticket_id = ?",
        (ticket_id,),
    )
    row = cursor.fetchone()
    conn.close()
...
def get_high_priority_tickets(service_name: str = None) -> dict:
    ...
    conn.close()
    tickets = [
        {"ticket_id": r[0], "service_name": r[1], "priority": r[2], "status": r[3], "subject": r[4]}
        for r in rows
    ]
    return {"count": len(tickets), "tickets": tickets}
