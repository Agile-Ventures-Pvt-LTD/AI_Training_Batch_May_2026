...
def clean_customer_id(cust_id: str) -> int:
    digits = re.sub(r"\D", "", str(cust_id))
...
def inspect_database_schema(dummy: str = "") -> dict:
    """retrieve the database schema, including tables and columns for each table."""
    try:
        tables = [
        "customer",
        "card",
...
def get_customer_profile(cust_id: str) -> dict:
    ...
    try:
        cust_id = clean_customer_id(cust_id)
