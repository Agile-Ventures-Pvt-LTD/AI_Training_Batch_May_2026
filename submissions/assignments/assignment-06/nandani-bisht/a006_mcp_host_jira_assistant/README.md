...
├── sample_queries.md
├── server/
│   └── jira_mcp_server.py     FastMCP server, stdio transport, 6 tools
├── src/
│   ├── host.py                Orchestration loop (FR-1 to FR-7)
│   ├── llm.py                 Groq chat-completions wrapper
│   ├── mcp_client.py          FastMCP Client wrapper (subprocess management)
│   └── prompts.py             System prompt (PRD section 17)
├── tests/
│   ├── conftest.py
