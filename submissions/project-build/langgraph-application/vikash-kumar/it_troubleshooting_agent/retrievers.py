...
def retrieve_by_issue(question,issue_policy):
    "THis will retreive by issue"
    vector_store = load_vector_store()
    docs = vector_store.similarity_search(query=question,k=TOP_K,filter={"policy_domain": issue_policy})
    return docs
...
def retrieve_policies(question,policy_domains):
    ...
    for domain in policy_domains:
        docs = retrieve_by_issue(question,domain)
