...
def get_resource(uri: str) -> str:
    """Returns static contextual resources based on target identifier strings."""
    if uri == "resource://travel/checklist":
        return CHECKLIST_CONTENT
    elif uri == "resource://travel/advisory-rules":
        return ADVISORY_RULES_CONTENT
    elif uri == "resource://weather/normalized-forecast-schema":
        return json.dumps(SCHEMA_CONTENT, indent=2)
    raise ValueError(f"Unknown Resource Identifier: {uri}")
